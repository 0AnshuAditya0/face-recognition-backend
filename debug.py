...
def get_face_encoding(image_path_or_array):
    ...
    try:
        ...
        print(f"  🔍 Detecting face in image size: {img.size}")
        ...
        print(f"  ✓ Found {len(boxes)} face(s), confidence: {probs[0]:.2f}")
        ...
        # Convert to tensor
        face_tensor = torch.tensor(np.array(face)).permute(2, 0, 1).float().div(255.0).unsqueeze(0).to(device)
...
def match_face(encoding, threshold=0.6):
    ...
    if not model_state['encodings']:
        print("  ⚠️ No encodings in database!")
        return "Unknown", 0
    
    best_match = None
    best_score = threshold
    
    print(f"  🔎 Comparing against {len(model_state['encodings'])} people...")
    ...
    if best_match:
        ...
        print(f"  ✓ Match found: {best_match} ({confidence}%)")
...
def load_lfw_dataset(max_images_per_person=10, save_interval=50):
    ...
    for person_name, img_count in valid_people[:100]:  # Process first 100 people for speed
        ...
        print(f"\n[{processed_count + 1}/100] Processing: {person_name} ({img_count} images)")
        training_state['current_step'] = f'Processing {person_name}...'
        training_state['processed_people'] = processed_count
        training_state['progress'] = 5 + int((processed_count / 100) * 85)
        ...
        if person_encodings:
            ...
            processed_count += 1
            print(f"  ✅ Added {len(person_encodings)} faces for {person_name}")
        
        # Save progress periodically
        if processed_count % save_interval == 0:
            save_model()
            print(f"\n💾 Progress saved: {processed_count}/100")
...
def save_model():
    ...
    try:
        ...
        print(f"💾 Model saved: {model_state['num_people']} people, {model_state['num_faces']} faces")
    except Exception as e:
...
async def health():
    return {
        ...
        'device': str(device),
        'dataset': 'LFW',
        'dataset_exists': os.path.exists(LFW_DIR)
    }
...
async def train(background_tasks: BackgroundTasks):
    ...
    if not os.path.exists(LFW_DIR):
        raise HTTPException(400, f"Dataset not found at {LFW_DIR}. Please run download_dataset.py first")
...
async def recognize_face(image: UploadFile = File(...)):
    ...
    if not model_state['trained']:
        ...
        raise HTTPException(400, "Model not trained yet. Please train the model first or add people to database.")
    
    print(f"✓ Model is trained: {model_state['num_people']} people in database")
    ...
    try:
        ...
        for i, (box, prob) in enumerate(zip(boxes, probs)):
            ...
            if prob < 0.9:
                print(f"  ⚠️ Skipping (low confidence)")
                continue
            
            # Get face encoding
            face_crop = pil_img.crop(box)
            face_crop = face_crop.resize((160, 160))
            face_tensor = torch.tensor(np.array(face_crop)).permute(2, 0, 1).float().div(255.0).unsqueeze(0).to(device)
        ...
        print(f"\n✅ Recognition complete: {len(results)} results")
        print("="*70 + "\n")
...
async def add_person(
    ...
    try:
        ...
        print(f"   Total people: {model_state['num_people']}")
        print(f"   Total faces: {model_state['num_faces']}")
        print(f"{'='*70}\n")
...
async def startup():
    print("\n" + "="*70)
    print("🎭 PyTorch Face Recognition System (DEBUG MODE)")
    print("="*70)
    print(f"🔧 Device: {device}")
    print(f"📁 Dataset: {LFW_DIR}")
    print(f"{'✅' if os.path.exists(LFW_DIR) else '❌'} Dataset exists: {os.path.exists(LFW_DIR)}")
    ...
    if load_model():
        ...
        print("   Either:")
        print("   1. Train model: POST /api/train")
        print("   2. Add people manually: POST /api/add-person")
